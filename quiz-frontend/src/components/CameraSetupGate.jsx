...
export default function CameraSetupGate({ onReady, onCancel, title = "System Check" }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d1117', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, color: 'var(--text-main)' }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div style={{ width: 64, height: 64, borderRadius: 18, background: 'rgba(37,99,235,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
          <Camera size={28} color="var(--primary-400)" />
        </div>
        <h2 style={{ fontSize: 24, fontWeight: 800, marginBottom: 8 }}>{title}</h2>
        <p style={{ fontSize: 14, color: 'var(--text-sec)', marginBottom: 24 }}>Please allow camera and microphone access, and align your face before starting.</p>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
          {onCancel && (
            <button onClick={handleCancel} style={{ padding: '12px 24px', borderRadius: 10, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-sec)', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
              <ArrowLeft size={15} style={{ marginRight: 6, verticalAlign: 'middle' }} /> Cancel
            </button>
          )}
          <button onClick={handleProceed} disabled={status !== 'ready'}
            style={{ padding: '12px 28px', borderRadius: 10, background: status === 'ready' ? 'linear-gradient(135deg,var(--primary),var(--primary-400))' : '#334155', border: 'none', color: '#fff', fontSize: 15, fontWeight: 700, cursor: status === 'ready' ? 'pointer' : 'not-allowed', opacity: status === 'ready' ? 1 : 0.5 }}>
            Start Exam
          </button>
        </div>
      </motion.div>
